...
PySide signal boundaries carrying source microseconds use Python object payloads rather than Qt `int` payloads. Qt maps `Signal(int)` to a signed 32-bit value, which cannot represent microseconds beyond approximately 35 minutes 47 seconds. Source identities, bounded timeline slider values, and transcript indices may remain ordinary Qt integers.
